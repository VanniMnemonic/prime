...
async function startUiServer(distRoot) {
...
function createWindow() {
    ...
    if (isDev) {
        ...
        const locale = getLocale();
        const distRoot = path.join(electron_1.app.getAppPath(), 'dist', 'prime', 'browser');
...
electron_1.app.on('ready', () => {
    ...
    data_source_1.AppDataSource.initialize().then(async () => {
        ...
        electron_1.ipcMain.handle('add-note', async (event, note) => {
            return await noteService.create(note);
        });
        electron_1.ipcMain.handle('delete-note', async (event, id) => {
...
electron_1.ipcMain.handle('update-user', async (event, userData) => {
    const userRepository = data_source_1.AppDataSource.getRepository(User_1.User);
    return await userRepository.save(userData);
});
electron_1.ipcMain.handle('get-titles', async () => {
    const titleRepository = data_source_1.AppDataSource.getRepository(Title_1.Title);
    return await titleRepository.find();
});
electron_1.ipcMain.handle('add-title', async (event, titleData) => {
    const titleRepository = data_source_1.AppDataSource.getRepository(Title_1.Title);
...
electron_1.ipcMain.handle('upload-image', async (event, filePath) => {
    try {
        const userDataPath = electron_1.app.getPath('userData');
...
electron_1.ipcMain.handle('get-assets', async () => {
    ...
    const assetsWithDetails = assets.map((asset) => {
        ...
        const inefficientQty = assetBatches.reduce((sum, batch) => sum + (batch.inefficient_quantity ?? 0), 0);
...
electron_1.ipcMain.handle('update-asset', async (event, assetData) => {
    const assetRepository = data_source_1.AppDataSource.getRepository(Asset_1.Asset);
    return await assetRepository.save(assetData);
});
electron_1.ipcMain.handle('get-batches-by-asset', async (event, assetId) => {
    const batchRepository = data_source_1.AppDataSource.getRepository(Batch_1.Batch);
...
electron_1.ipcMain.handle('update-batch', async (event, batchData) => {
    const batchRepository = data_source_1.AppDataSource.getRepository(Batch_1.Batch);
    return await batchRepository.save(batchData);
});
electron_1.ipcMain.handle('get-batch-by-serial', async (event, serialNumber) => {
    const batchRepository = data_source_1.AppDataSource.getRepository(Batch_1.Batch);
...
electron_1.ipcMain.handle('add-withdrawal', async (event, withdrawalData) => {
    const queryRunner = data_source_1.AppDataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
        const { asset, batch, quantity, user, date, must_return, expected_return_date } = withdrawalData;
        ...
        if (batch) {
            ...
            const existingBatch = await queryRunner.manager.findOne(Batch_1.Batch, { where: { id: batch.id } });
            if (!existingBatch)
            ...
            const availableBatches = await queryRunner.manager.find(Batch_1.Batch, {
            ...
            const now = new Date();
            const validBatches = availableBatches.filter((b) => b.quantity > 0 && (!b.expiration_date || new Date(b.expiration_date) >= now));
...
electron_1.ipcMain.handle('return-withdrawal', async (event, { id, date, returnedQuantity, inefficientQuantity }) => {
    const queryRunner = data_source_1.AppDataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
        const withdrawalRepository = queryRunner.manager.getRepository(Withdrawal_1.Withdrawal);
...
electron_1.ipcMain.handle('force-return-withdrawal', async (event, { id, date, returnedQuantity }) => {
    const queryRunner = data_source_1.AppDataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
        ...
        const allBatches = await queryRunner.manager.find(Batch_1.Batch, {
        ...
        const now = new Date();
        const nonExpiredBatches = allBatches.filter((b) => !b.expiration_date || new Date(b.expiration_date) >= now);
...
electron_1.ipcMain.handle('reset-db', async () => {
    const queryRunner = data_source_1.AppDataSource.createQueryRunner();
    await queryRunner.connect();
